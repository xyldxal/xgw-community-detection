...
Modified on 2023-04-21 by Anthony Van Cayetano
Changes:
    - This modified file is the Python 3 version of the original
        ...
        with the name 'tcss.py.bak'.
    
    - An indentation issue was fixed by Anthony Van Cayetano, as annotated
        in this file (see comment annotation below).

    - The result variable in the input_file_run function was also modified,
        as annotated below.
...
def _command_line_arguments(argv):
    ...
    for opt, arg in opts:
        if opt in ("-h", "--help"):
            ...
            options['detail'] = True
        elif opt in ("--drop"): # Modified by Anthony Van Cayetano on 2023-04-14. Fixed the identation of this line and the line below.
            options['drop'] = arg
        elif opt in ("--gene"):
            if os.path.isfile(arg):
                options['gene'] = arg
...
def input_file_run(objs, infile, outfile, detail):
    ...
    '''
    file = open(infile, 'r')
    result = "PROTEIN_U,PROTEIN_V,GO_CC,GO_BP,GO_MF\n" # Modified by Anthony Van Cayetano on 2023-04-21.
    for line in file:
